...
class PersonalAssistant:
    ...
    def add_contact_from_console(self):
        ...
        while True:
            try:
                ...
                birthday_date = parser.parse(birthday).date()                   # Перетворюємо рядок на datetime object
    ...
    def list_contacts(self):
        ...
        if not self.contacts:
            ...
            for contact in self.contacts:
                ...
                birthday_str = contact.birthday.strftime('%d-%m-%Y')                # CHANGED FROM     birthday_str = contact.birthday.strptime('%d-%m-%Y').date()
        ...
        with open('addressbook.csv', 'w', newline='\n') as fh:
            ...
            for contact in self.contacts:
                writer.writerow({'name': contact.name, 'address': contact.address,
                                 'phone': contact.phone, 'email': contact.email, 'birthday': contact.birthday.strftime('%d-%m-%Y')})        # CHANGED FROM <'birthday': contact.birthday>
    ...
    def upcoming_birthdays(self, days):
        ...
        if not upcoming_birthdays:
            console.print(f'[yellow]Протягом наступних {days} днів не буде жодного дня народження.[/yellow]')
        else:
            table = Table(title=f'Дні народження впродовж наступних {days} днів')
            table.add_column("[blue]Ім'я[/blue]")
    ...
    def edit_contact(self, contact):
        ...
        new_birthday = input(
            f"Теперішній день народження: {contact.birthday.strftime('%d-%m-%Y')}\nВведіть новий день народження (або Enter, щоб залишити без змін): ")  # CHANGED FROM {contact.birthday}
        if new_birthday:
            contact.birthday = parser.parse(new_birthday).date()
    ...
    def search_notes(self, text_query=None, tag_query=None):
        specify_query = input(
            "Для пошуку за текстом введіть слово 'текст' або слово 'тег' для пошуку за тегом: ")
    ...
    def analyze_user_input(self, user_input):
        ...
        if "додати контакт" in normalized_input:
            ...
            console.print(
                "[green]Ви можете використовувати команду дні народження для пошуку контактів у яких буде день народження.[/green]")
            ...
            console.print("[green]Ви можете використовувати команду пошук нотаток для пошуку нотаток за текстом або за тегом.[/green]")
