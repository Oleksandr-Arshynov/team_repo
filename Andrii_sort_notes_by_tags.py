...
class PersonalAssistant:
    def __init__(self):
        self.contacts = []
        self.notes = []
        self.commands = ['додати контакт', 'список контактів', 'пошук контактів', 'дні народження', 'додати нотатку', 'список нотаток', 'пошук нотаток']
    ...
    def add_note(self, text=None, tags=None):                           # text and tags are not necessary
        while True:
            text = input("Введіть нотатку або введіть 'вийти' для завершення: ")
            if text == 'вийти':
                break
            else:
